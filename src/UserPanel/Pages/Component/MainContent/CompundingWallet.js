...
const CompundingModal = ({ open, setOpen , setcompound }) => {
  ...
  async function PayinZp() {
    const reqbody = {
      amount: Number(fk.values.inr_value),
    };
    try {
      const res = await axiosInstance.post(API_URLS?.member_compound_request,
        reqbody,
      );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white w-[90%] max-w-md rounded-xl shadow-lg p-6 relative">
        ...
        <label className="text-sm text-slate-600">Compunding Wallet</label>
        <input
          type="number"
          value={setcompound}
...
export default CompundingModal;
